...
def infotodict(seqinfo):
    ...
    for s in seqinfo:
        ...
        if any(x.lower() in s.series_description.lower() for x in ('MPRAGE','T1')):
            info[t1w].append(s.series_id)
        elif any(x.lower() in s.series_description.lower() for x in ('T2W','T2')) and 'fse' not in s.series_description.lower():
            info[t2w].append(s.series_id)
        # the less straightforward (mixed) series
        elif any(x.lower() in s.series_description.lower() for x in ('T2W','T2')) and 'fse' in s.series_description.lower():
            if s.dim3 < 40:
                info[t2w].append(s.series_id)
            else:
                info[pdt2].append(s.series_id)
        elif any(x.lower() in s.series_description.lower() for x in ('AX','AXIAL')) and any(x in s.series_description.lower() for x in ('tse','pd')):
            info[pdt2].append(s.series_id)
        elif any(x.lower() in s.series_description.lower() for x in ('FLAIR')):
            info[flair].append(s.series_id)
        elif any(x.lower() in s.series_description.lower() for x in ('ep2d')):
            info[bold].append(s.series_id)
        elif any(x.lower() in s.series_description.lower() for x in ('DTI')):
...
def custom_callable(*args):
    ...
    for echo, (nifti, json) in zip(echonums, bids_pairs):
        ...
        prov_file = prefix
        embed_metadata_from_dicoms(True, dicoms,
